...
class BaseDataLoader(ABC):
    ...
    def __init__(self, params: DataLoader, process_lib: str = "pandas"):
        ...
        self._val_df = None
        self._test_df = None
        self.account_feature_manager = self._get_account_feature_manager()
        self.user_feature_manager = self._get_user_feature_manager()
        self.item_feature_manager = self._get_item_feature_manager()
        self.interacted_feature_manager = self._get_interacted_feature_manager()
        self.spark_operation = self.interacted_feature_manager.spark_operation
    ...
    def _get_item_feature_manager(self):
        return FeatureManagerCollection().item_fm[self.process_lib]

    def _get_account_feature_manager(self):
        return FeatureManagerCollection().account_fm[self.process_lib]

    def _get_user_feature_manager(self):
        return FeatureManagerCollection().user_fm[self.process_lib]

    def _get_interacted_feature_manager(self):
        return FeatureManagerCollection().interacted_fm[self.process_lib]
