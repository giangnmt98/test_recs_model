...
class PytorchDataLoader(BaseDataLoader):
    ...
    def extract_main_dataframe(
        ...
        filters: List[str],
        is_train: bool,
        user_feature_df: Union[pd.DataFrame, DataFrame],
        item_feature_df: Union[pd.DataFrame, DataFrame],
        ...
        if self.process_lib == "pyspark":
            big_df = self.interacted_feature_manager.extract_dataframe(
                features_to_select=interacted_cols, filters=filters
            )

            if self.time_order_event_col == "":
                big_df = big_df.drop_duplicates()

            df_pop_items = self.get_df_pop_items(filters, item_feature_df, is_train)

            df = FeatureAssembly().assemble(big_df, user_feature_df, item_feature_df)
            ...
            self.interacted_feature_manager.process_lib = "pandas"
            big_df = self.interacted_feature_manager.extract_dataframe(
                features_to_select=interacted_cols + [self.batch_idx_col],
                filters=filters,
            )
            # revert for interacted_feature_manager to self.process_lib
            self.interacted_feature_manager.process_lib = self.process_lib
            ...
            for batch_idx, df in tqdm(big_df.groupby(self.batch_idx_col)):
                ...
                df = FeatureAssembly().assemble(df, user_feature_df, item_feature_df)
    ...
    def get_df_pop_items(self, filters, item_feature_df, is_train):
        if self.process_lib == "pyspark":
            ...
            if is_train:
                big_df = (
                    self.interacted_feature_manager.extract_dataframe(
                        features_to_select=[self.item_id_col, self.time_order_event_col]
                        + [self.batch_idx_col, self.popularity_item_group_col],
                        filters=filters + [f"{self.label_col} == 2"],
                    )
                ...
                df_pop_items = FeatureAssembly().assemble(
                    big_df,
                    cudf.DataFrame(),
                    item_feature_df,
                )
    ...
    def extract_dataframe(
        ...
        account_feature_df = self.account_feature_manager.extract_dataframe(
            features_to_select=[self.user_id_col] + account_cols,
        )
        profile_feature_df = self.user_feature_manager.extract_dataframe(
            features_to_select=[self.user_id_col] + profile_cols,
        )
        ...
        item_feature_df = self.item_feature_manager.extract_dataframe(
            features_to_select=[self.item_id_col] + item_cols,
            filters=self.params.content_filters,
        )
