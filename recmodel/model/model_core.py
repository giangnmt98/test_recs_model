...
from recmodel.model.model_modules import DLRMModule, EmbeddingModule, LightSE, NCFModule
...
class DHashNCF(BaseDeepLearningModel):
    def __init__(self, config: DLModelParams):
        super().__init__(config)
        self.concatenated_embedding_size = sum(
            sparse_feature.num_factors
            for sparse_feature in config.sparse_features
            if sparse_feature.name not in ["hashed_user_id_v2", "hashed_item_id_v2"]
            and sparse_feature.enable
        )
        hidden_layers_size_list = [
            self.concatenated_embedding_size
        ] + config.hidden_layers
        self.embedding = EmbeddingModule(config.sparse_features)
        self.ncf = NCFModule(
            hidden_layers_size_list,
...
class DHashDLRM(BaseDeepLearningModel):
    def __init__(self, config: DLModelParams):
        ...
        self.embedding = EmbeddingModule(config.sparse_features)
        self.dlrm = DLRMModule(
            hidden_layers_size_list,
            config.activation_function,
        )

    def forward(self, x):
        all_embeddings = self.embedding(x)
        stack_emb = torch.cat([emb.unsqueeze(1) for emb in all_embeddings], axis=1)
        x = self.dlrm(stack_emb)
        return x
