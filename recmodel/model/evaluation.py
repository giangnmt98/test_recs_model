...
from recmodel.base.utils.utils import (
    ...
    get_date_before,
    get_date_filters,
    merge_profile_features_with_account_features,
    return_or_load,
)
...
class Evaluation:
    ...
    def __init__(
        ...
        self.model_pipeline_config_path = model_pipeline_config_path
        self.num_days_to_train = num_days_to_train
        self.pipeline = self._get_pipeline()
        self.model_root_dir = Path(model_root_dir)
        self.account_fm = self.pipeline.data_loader.account_feature_manager
        self.user_fm = self.pipeline.data_loader.user_feature_manager
        self.item_fm = self.pipeline.data_loader.item_feature_manager
        self.interacted_fm = self.pipeline.data_loader.interacted_feature_manager
        ...
        self.spark_operation = self.interacted_fm.spark_operation
    ...
    def _get_pipeline(self) -> PytorchPipeline:
        return PytorchPipeline(
            self.model_pipeline_config_path, process_lib=self.process_lib
        )
    ...
    def _get_df_to_eval_and_df_to_score(self, for_date: int) -> pd.DataFrame:
        ...
        account_feature_cols = self.pipeline.data_loader.account_feature_names
        profile_feature_cols = self.pipeline.data_loader.profile_feature_names
        item_cols = self.pipeline.data_loader.item_feature_names

        account_feature_df = self.account_fm.extract_dataframe(
            features_to_select=[self.user_id] + account_feature_cols,
        )
        profile_feature_df = self.user_fm.extract_dataframe(
            features_to_select=[self.user_id] + profile_feature_cols,
        )

        user_feature_df = merge_profile_features_with_account_features(
            profile_feature_df, account_feature_df, self.user_id
        )

        date_to_eval = get_date_before(for_date, 1)

        item_feature_df = self.item_fm.extract_dataframe(
            features_to_select=[self.item_id] + item_cols,
            filters=[f"{const.FILENAME_DATE_COL} <= {date_to_eval}"],
        )
        ...
        df = self.interacted_fm.extract_dataframe(
        ...
        interacted_df = self.interacted_fm.extract_dataframe(
            features_to_select=id_cols,
            filters=interacted_date_filters,
        ).drop_duplicates()
        df_to_eval = self._drop_train_pairs_and_assemble(
            df, interacted_df, user_feature_df, item_feature_df, date_to_eval
        )
        ...
        df_to_score = (
            ...
            .assign(key=1)
            .merge(
                interacted_df[[self.item_id]].drop_duplicates().assign(key=1), on="key"
            )
            .drop("key", axis=1)
        )
        df_to_score = self._drop_train_pairs_and_assemble(
            df_to_score, interacted_df, user_feature_df, item_feature_df, date_to_eval
        )

        return df_to_score, df_to_eval

    def _drop_train_pairs_and_assemble(
        self, df, interacted_df, user_feature_df, item_feature_df, date_to_eval
    ):
    ...
    def _evaluate_each_model_checkpoint(
        ...
        """
        model_analysis_config = return_or_load(
            self.model_pipeline_config_path, PipelineConfig, parse_pipeline_config
        ).model_analysis
